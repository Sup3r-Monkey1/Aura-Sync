...
export default function AuraDashboard() {
  const { readiness, muscleHeat, history, totalWorkouts, watchConnected, connectWatch, evolutionXP, nutrition, refreshReadiness, decayHeatmap, startSession, session, hardReset } = useWorkoutStore();
  ...
  // REAL LOGGED STATS (Starting at 0)
    ...
    <div className="min-h-screen bg-[#050505] pb-24">
      <div className="px-4 pt-12 pb-6 flex justify-between items-end">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-lg font-black tracking-widest text-white/90 uppercase">Aura Sync</h1>
          </div>
          <p className="text-[10px] text-white/30 uppercase tracking-[0.2em]">Bio-Link: {overallFatigue}% System Stress</p>
        </div>
        <div className="text-right">
          <div className="text-[10px] text-white/20 uppercase font-mono tracking-widest">XP</div>
      ...
      <div className="px-4 mb-6">
        <div className="glass p-5">
          ...
          {overallFatigue === 0 ? (
            <div className="py-10 text-center"><div className="text-[10px] text-white/10 uppercase tracking-[0.3em] italic">Awaiting First Session Log</div></div>
          ) : (
      ...
      {!session && (
        <div className="px-4 mb-8">
          <motion.button whileTap={{ scale: 0.97 }} onClick={() => startSession()} className="w-full py-5 bg-cobalt text-black font-black text-sm uppercase tracking-widest glow-cobalt">Initiate Workout Sync</motion.button>
        </div>
      )}
      ...
      <div className="px-4 mt-20 pb-12 text-center">
        <button onClick={() => { if (window.confirm("⚠️ SYSTEM WIPE: Delete all data?")) hardReset(); }} className="text-[9px] font-black text-red-500/30 hover:text-red-500 uppercase tracking-widest transition-colors">Hard Reset System</button>
      </div>
    </div>
  );
}
