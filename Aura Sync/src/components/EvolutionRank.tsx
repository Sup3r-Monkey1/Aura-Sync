...
export default function EvolutionRank() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] p-4 pt-10 pb-24 overflow-x-hidden">
      ...
      <div className="glass-strong p-10 mt-6 mb-6 flex flex-col items-center relative overflow-hidden border-t border-white/10 rounded-3xl">
      ...
      <div className="grid grid-cols-3 gap-2 mb-8 font-mono">
         <div className="glass p-4 text-center rounded-xl"><Heart size={14} className={watchConnected ? "text-red-500 animate-pulse" : "text-white/5"} /><div className="text-lg font-black mt-2 text-white aura-text-glow">{watchConnected ? "64" : "--"}</div><div className="text-[7px] text-white/20 uppercase font-bold mt-1">Bio_Sync</div></div>
         <div className="glass p-4 text-center border-b border-cobalt/40 rounded-xl"><Activity size={14} className="text-cobalt" /><div className="text-lg font-black mt-2 text-white aura-text-glow">{history.length}</div><div className="text-[7px] text-white/20 uppercase font-bold mt-1">Uploads</div></div>
         <div className="glass p-4 text-center rounded-xl"><ShieldCheck size={14} className="text-terminal" /><div className="text-lg font-black mt-2 text-white">{Math.min(100, history.length*5)}%</div><div className="text-[7px] text-white/20 uppercase font-bold mt-1">Status</div></div>
      </div>
      {!watchConnected && <button onClick={connectWatch} className="w-full py-5 bg-white/5 border border-white/10 text-white/40 font-black uppercase text-[8px] tracking-[0.4em] active:bg-cobalt active:text-black transition-all rounded-xl shadow-lg font-mono">Establish Hardware Bio-Link</button>}
    </div>
  );
}
