...
export default function EvolutionRank() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] p-4 pt-12 pb-24 overflow-x-hidden">
      <h2 className="text-sm font-black uppercase tracking-[0.4em] text-white/70 italic px-2">Anatomical_Evolution</h2>
      ...
      <div className="glass-strong p-10 mt-8 mb-8 flex flex-col items-center relative overflow-hidden border-t border-white/10 rounded-3xl">
        <div className="relative w-72 h-[450px] flex justify-center">
           <svg viewBox="0 0 200 400" className="w-full h-full">
              ...
              <rect x="85" y="155" width="30" height="40" fill={getMColor('core')} rx="2" />
              {/* BACK / SHOULDERS */}
      ...
      <div className="grid grid-cols-3 gap-3 mb-10 font-mono">
         ...
         <div className="glass p-5 text-center border-b border-cobalt/40"><Activity size={16} className="text-cobalt" /><div className="text-xl font-black mt-2 text-white">{history.length}</div><div className="text-[8px] text-white/20 uppercase font-bold tracking-widest mt-1">Logs</div></div>
         <div className="glass p-5 text-center"><ShieldCheck size={16} className="text-terminal" /><div className="text-xl font-black mt-2 text-white">{Math.min(100, history.length*5)}%</div><div className="text-[8px] text-white/20 uppercase font-bold tracking-widest mt-1">Verify</div></div>
      </div>
      {!watchConnected && <button onClick={connectWatch} className="w-full py-5 bg-white/5 border border-white/10 text-white/40 font-black uppercase text-[10px] tracking-[0.4em] active:bg-cobalt active:text-black transition-all">Establish Hardware Link</button>}
    </div>
  );
}
