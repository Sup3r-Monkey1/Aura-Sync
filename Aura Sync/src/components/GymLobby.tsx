...
export default function GymLobby() {
  const { totalWorkouts, history, ghostVolume, userName } = useWorkoutStore();
  ...
  return (
    <div className="min-h-screen bg-[#050505] p-4 pt-12 pb-24">
      <div className="flex items-center justify-between mb-8 px-2">
        ...
        <button onClick={() => setShowCalendar(true)} className="px-5 py-2 bg-white/5 border border-white/10 active:scale-95 transition-all">
          <div className="flex items-center gap-2"><CalIcon size={12} className="text-cobalt" /><span className="text-[10px] font-black uppercase text-white/80">Monthly_Link</span></div>
        </button>
      </div>

      <div className="glass-strong p-8 border-t border-white/10 mb-8 relative overflow-hidden">
         <div className="flex justify-between items-center mb-10">
            <div className="flex items-center gap-2"><Swords size={18} className="text-magenta animate-pulse" /><span className="text-xs font-black text-white uppercase tracking-[0.3em]">Neural Rivalry</span></div>
            <div className="text-[10px] font-mono text-magenta/60 uppercase">{isWinning ? 'DOMINANT' : 'SYNCING...'}</div>
         </div>
         <div className="space-y-10 relative z-10">
            <div className="relative">
               <div className="flex justify-between text-[10px] mb-3 font-black uppercase tracking-[0.2em]">
                  <span className="text-white/40 italic flex items-center gap-2"><Zap size={10} className="text-cobalt"/> {userName}</span>
               ...
               <div className="h-1.5 bg-white/5 overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${myProgress}%` }} className="h-full bg-cobalt shadow-[0_0_15px_#2563eb]" transition={{ duration: 1.5, ease: "circOut" }}/></div>
            </div>
            <div className="relative">
               <div className="flex justify-between text-[10px] mb-3 font-black uppercase tracking-[0.2em]">
               ...
               <div className="h-1.5 bg-white/5 overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${ghostProgress}%` }} className="h-full bg-magenta/40" transition={{ duration: 1.5, ease: "circOut" }}/></div>
            </div>
         </div>
         <div className="absolute inset-0 bg-gradient-to-b from-white/[0.02] to-transparent h-1/2 animate-scan pointer-events-none" />
         {isWinning && (
           <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} onClick={() => setShowDominance(true)} className="w-full mt-10 py-4 bg-magenta text-black text-[10px] font-black uppercase tracking-[0.4em] shadow-[0_0_30px_rgba(229,53,171,0.3)]">Claim Dominance</motion.button>
         )}
      </div>
      ...
      <AnimatePresence>
        ...
        {showDominance && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[150] bg-black/98 flex flex-col items-center justify-center p-6 text-center" onClick={() => setShowDominance(false)}>
             <motion.div initial={{ scale: 0.5 }} animate={{ scale: 1 }} transition={{ type: 'spring' }} className="mb-6"><Crown size={80} className="text-magenta text-glow-magenta" /></motion.div>
             <h2 className="text-4xl font-black italic text-magenta tracking-tighter mb-2 uppercase">Lobby Dominance</h2>
             <p className="text-xs text-white/40 uppercase tracking-[0.3em]">Protocol Verified</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
